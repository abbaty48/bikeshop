...
const ZoomButton = memo(function ZoomButton(props: { title: string, 'aria-label': string, disabled: boolean, onClick: React.MouseEventHandler<HTMLButtonElement> } & PropsWithChildren) {
    return (
        <button type="button" {...props}
            className="disabled:pointer-events-none cursor-pointer hover:scale-90 transition-all">
            {props.children}
        </button>
    )
})
...
export const ZoomControls = memo(function ZoomControls(props: TProps) {
    ...
    return (<menu className={`grid grid-cols-3 gap-8 md:grid-cols-none md:grid-rows-3 place-self-center md:place-self-auto p-2 md:p-4 max-w-max text-2xl md:text-3xl order-1 md:order-none ${!props.isFailed && 'bg-c1'}`}>
